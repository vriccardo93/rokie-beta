...
def api(path, payload=None, timeout=16):
    url=f'{BASE}/{path}'
    data=None if payload is None else json.dumps(payload).encode()
    req=Request(url,data=data,headers={'Accept':'application/json','Content-Type':'application/json'})
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path=='/api/health':
            try: api('tipologiche/estensioni'); self.sendj({'app':'ok','normattiva':True,'message':'Normattiva Open Data raggiungibile'})
            except Exception as e: self.sendj({'app':'ok','normattiva':False,'message':type(e).__name__})
            return
